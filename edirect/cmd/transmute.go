...
func main() {
	...
	*/
	...
	// CONVERT IDENTIFIERS AND PARENTS TO IDENTIFIERS AND LINEAGE

	doParentsToLineage := func() {

		tble := eutils.ParentsToLineage(in)

		if tble == nil {
			eutils.DisplayError("Unable to create parents to lineage converter")
			os.Exit(1)
		}
	...
	if len(args) > 0 && args[0] == "-g2x" {
		...
		*/
		...
		arg, ok := nextArg()
		if ok {
			if arg == "-express" || arg == "express" {
				// low memory, use only one converter, unshuffler does not buffer
				express = true
			}
		}
		...
		gbkq := eutils.CreateGBConverters(psrq, express)
		unsq := eutils.CreateTextUnshuffler(gbkq, express)
...
`
	...
	if len(args) > 0 && args[0] == "-gbf" {
		...
		unsq := eutils.CreateTextUnshuffler(psrq, false)
