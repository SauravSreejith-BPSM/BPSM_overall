...
// LclArchPathMap allows GetLocalPath method on this map type
type LclArchPathMap map[string]string

// GetLocalPath returns the path to a specific folder and a flag that checks if the folder is mounted
func (p LclArchPathMap) GetLocalPath(fldr string) (string, bool) {
...
// ResolveArchivePaths returns a map with paths to local archive component directories
func ResolveArchivePaths(db string) LclArchPathMap {
	...
	// eventually remove the default assignment above to require explicit database argument
	if db == "" {
		DisplayError("Required database argument not supplied")
		os.Exit(1)
	}
	...
	// top-level groups (ARCHIVE, WORKING, POSTING, and SOURCES) allow separate volumes or
	// paths for related subfolders, read from configuration file or environment variables
	topPaths := make(map[string]string)
	if topPaths == nil {
		return nil
	}

	getIniFile := func(cfg string) string {

		if cfg == "" {
			return ""
		}

		inFile, err := os.Open(cfg)
		if err != nil {
			DisplayError("Unable to open configuration file '%s'", cfg)
			return ""
		}
		defer inFile.Close()
	...
	getEnvPath := func(env string) string {
	...
	// first try configuration file specified by environment variable
	config := os.Getenv("EDIRECT_LOCAL_CONFIG")
	if config != "" {

		/*
		   [pubmed]
		   ARCHIVE=/Volumes/archive/
		   POSTING=/Volumes/posting/
		   SOURCES=/Volumes/sources/
		   WORKING=/Volumes/working/
		*/

		text := getIniFile(config)
		if text != "" {

			pat := ParseRecord(text[:], "ConfigFile")
			if pat != nil {

				// limit to database section
				VisitNodes(pat, db, func(sect *XMLNode) {

					// set top-level group paths from configuration section
					for _, dr := range []string{"ARCHIVE", "WORKING", "POSTING", "SOURCES"} {
						VisitElements(sect, dr, "", func(str string) { topPaths[dr] = strings.TrimSuffix(str, "/") })
					}
				})
			}
		}
	...
	// if ARCHIVE path was not specified in configuration file
	if topPaths["ARCHIVE"] == "" {

		// clear any values in map
		clear(topPaths)

		// set group paths from specific environment variable
		topPaths["ARCHIVE"] = getEnvPath("EDIRECT_LOCAL_ARCHIVE")
		topPaths["WORKING"] = getEnvPath("EDIRECT_LOCAL_WORKING")
		topPaths["POSTING"] = getEnvPath("EDIRECT_LOCAL_POSTING")
		topPaths["SOURCES"] = getEnvPath("EDIRECT_LOCAL_SOURCES")
	}

	// at minimum, ARCHIVE path is required
	if topPaths["ARCHIVE"] == "" {
		DisplayError("Configuration does not specify the required ARCHIVE path")
		os.Exit(1)
	}

	// next, fill in any unassigned top-level paths from defaults
	// if WORKING is set, only Archive, Data, and Postings directories remain in ARCHIVE path
	if topPaths["WORKING"] == "" {
		topPaths["WORKING"] = topPaths["ARCHIVE"]
	}
	// Postings directory can also be moved to a separate location, to save space on individual solid-state drives
	if topPaths["POSTING"] == "" {
		topPaths["POSTING"] = topPaths["ARCHIVE"]
	}
	// Source files can also be in a separate location, including on a rotating hard drive, since record access is
	// sequential, and only needed at the first step of caching as individual records
	if topPaths["SOURCES"] == "" {
		topPaths["SOURCES"] = topPaths["WORKING"]
	}

	// populate map of subfolder access paths based on four top-level configured paths
	// Config folder is likely to be added to the ARCHIVE section in the future
	for _, dr := range []string{"Archive", "Data"} {
		pth := topPaths["ARCHIVE"] + "/" + db + "/" + dr
		res[dr] = pth
	}

	res["Sentinels"] = topPaths["ARCHIVE"] + "/" + db + "/Archive/Sentinels"

	for _, dr := range []string{"Extras", "Index", "Invert", "Merged", "Scratch"} {
		pth := topPaths["WORKING"] + "/" + db + "/" + dr
		res[dr] = pth
	}

	for _, dr := range []string{"Current", "Indexed", "Inverted"} {
		pth := topPaths["WORKING"] + "/" + db + "/Scratch/" + dr
		res[dr] = pth
	}

	res["Postings"] = topPaths["POSTING"] + "/" + db + "/Postings"

	res["Source"] = topPaths["SOURCES"] + "/" + db + "/Source"

	return res
}

// ReturnArchivePaths returns an XML string with paths to local archive component directories
func ReturnArchivePaths(db string) string {
	...
	// require at least Archive path for record caching and retrieval
	archiveBase, _ := pths.GetLocalPath("Archive")
	...
	buffer.WriteString("<ArchivePaths>\n")

	buffer.WriteString("  <Database>")
	buffer.WriteString(db)
	buffer.WriteString("</Database>\n")

	// sort presentation order alphabetically by path
	var keys []string
	for ky := range pths {
		keys = append(keys, ky)
	}
	sort.Slice(keys, func(i, j int) bool {
		a := keys[i]
		b := keys[j]
		return pths[a] < pths[b]
	})

	for _, ky := range keys {
		fldr := pths[ky]
		buffer.WriteString("  <")
		buffer.WriteString(ky)
		buffer.WriteString(">")
		buffer.WriteString(fldr)
		buffer.WriteString("</")
		buffer.WriteString(ky)
		buffer.WriteString(">\n")
	}
...
func evaluateQuery(base, db, phrase string, clauses []string, noStdout, isLink bool) (int, []int32) {
	...
	phrasePositions := func(pn, pm []uint32, dlt uint32) []uint32 {
	...
	proximityPositions := func(pn, pm []uint32, dlt uint32) []uint32 {
	...
	eval := func(str string) ([]int32, [][]uint32, int) {
	...
	for _, str := range clauses {
		...
		if strings.HasSuffix(str, " [YEAR]") {
			...
			if meshName.fpath == "" || meshTree.fpath == "" {
				pths := ResolveArchivePaths("pubmed")
				if pths == nil {
					DisplayError("Unable to get local archive configuration paths")
					os.Exit(1)
				}

				dataBase, _ := pths.GetLocalPath("Data")
				if dataBase != "" {
					if meshName.fpath == "" {
						meshName.fpath = filepath.Join(dataBase, "meshname.txt")
					}
					if meshTree.fpath == "" {
						meshTree.fpath = filepath.Join(dataBase, "meshtree.txt")
					}
				}
			}
...
func ProcessSearch(db, phrase string, xact, titl, isLink, deStop bool) int {
	...
	postingsBase, _ := pths.GetLocalPath("Postings")
...
func TermCounts(db, field, key, ttls string) int {
	...
	dpath := filepath.Join(postingsBase, field, ttls)
...
func ProcessLinks(db, fld string, ranked bool) {
	...
	combinedLinks := make(map[int32]int)
					...
					for _, term := range terms {
						...
						linkLoop := func(offset, size int32) {
							...
							for _, uid := range data {
								val := combinedLinks[uid]
								val++
								combinedLinks[uid] = val
							}
...
func ProcessMatch(db, phrase string, deStop bool) {
	...
	pentaPeptides := func(titl string) []string {

		var arry []string

		titl = strings.ToLower(titl)

		// break string into overlapping peptide pentamers
		words := SlidingSlices(titl, 5)

		for _, item := range words {
			if item == "" {
				continue
			}
			// do not skip stop words
			arry = append(arry, item)
		}
