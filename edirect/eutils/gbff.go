				...
				for {
					...
					for {
						...
						if strings.HasPrefix(txt, "/") {
							...
							if qual == "calculated_mol_wt" || qual == "mol_wt" {
								hasMolWt = true
							}
							...
							if qual == "translation" && !hasMolWt {
								mw := ProteinWeight(val, true, false)
								if mw != "" {
									rec.WriteString("          <INSDQualifier>\n")
									writeOneElement("            ", "INSDQualifier_name", "calculated_mol_wt")
									writeOneElement("            ", "INSDQualifier_value", mw)
									rec.WriteString("          </INSDQualifier>\n")
									hasMolWt = true
								}
							}
...
func CreateGBConverters(inp <-chan TextRecord, express bool) <-chan TextRecord {
	...
	converterCount := numServe
	if express {
		// low memory, use only one converter
		converterCount = 1
	}
