...
// Atlas consolidates several maps and one structure pointer
type Atlas struct {
...
func processClause(
	...
	transform := atl.Transform
	srchr := atl.Srchr
	histogram := atl.Histogram
	...
	processElement := func(acc func(string)) {
		...
		for _, stage := range stages {
			...
			case LEVEL:
				// -element "^" prints depth of current XML object
			...
			case QUESTION:
				// -element "?" prints name of current XML object
				acc(curr.Name)
			case TILDE:
				// -element "~" prints contents of current XML object
				if curr.Contents != "" {
					acc(html.UnescapeString(curr.Contents))
				} else if curr.Children != nil && doMixed {
					str := getMixedContentString(curr)
					if str != "" {
						acc(str)
					}
				}
	...
	case INDEXER:
		...
		okayToSkip := false
		switch label {
		case "TITL", "TIAB", "ABST", "TERM", "TEXT", "STEM":
			// only allow stop word removal for a defined set of text fields
			okayToSkip = true
		default:
			// do not remove stop words for sequence letters or fragments
		}
		...
		processElement(func(str string) {
			...
			*/
			...
			for _, item := range words {
				...
				// optional stop word removal - suppressed for sequence letters or fragments
				if deStop && okayToSkip && IsStopWord(item) {
					continue
				}
...
func processInstructions(
	...
	for _, op := range commands {
		...
		case ELEMENT:
			...
			txt, ok := processClause(curr, op.Stages, mask, cls, op.Type, index, level, atl)
			if ok {
				if varname != "" {
...
func conditionsAreSatisfied(conditions []*Operation, curr *XMLNode, mask string, index, level int, variables map[string]string) bool {
	...
	matchFound := func(stages []*Step) bool {
		...
		testConstraint := func(str string) bool {
			...
			switch stat {
			case EQUALS, CONTAINS, MIMICS, INCLUDES, EXCLUDES, ISWITHIN, STARTSWITH, ENDSWITH, ISNOT, ISBEFORE, ISAFTER, CONSISTSOF, MATCHES, RESEMBLES:
				...
				case MIMICS:
					...
					str = strings.Map(puncToSpace, str)
					val = strings.Map(puncToSpace, val)
					str = CompressRunsOfSpaces(str)
					val = CompressRunsOfSpaces(val)
					if strings.Contains(str, val) {
						return true
					}
				case INCLUDES:
				...
				case MATCHES:
					...
					val = TransformAccents(val, false, false)
					str = RemoveCommaOrSemicolon(str)
					val = RemoveCommaOrSemicolon(val)
					if strings.ToLower(str) == strings.ToLower(val) {
						return true
					}
				case RESEMBLES:
					str = SortStringByWords(str)
					val = SortStringByWords(val)
					if strings.ToLower(str) == strings.ToLower(val) {
						return true
					}
				default:
		...
		case LEVEL:
			// depth of explored parent object
			number = strconv.Itoa(level)
			found = true
		case INDEX:
		...
		case TILDE:
			// use object value
			if curr.Contents != "" {
				str := html.UnescapeString(curr.Contents)
				if constraint == nil || checkConstraint(str) {
					found = true
				}
			} else if curr.Children != nil && doMixed {
	...
	processNode := func(node *XMLNode, idx, lvl int) {
		...
		if conditionsAreSatisfied(cmds.Conditions, node, match, idx, lvl, variables) {
			...
			if len(cmds.Commands) > 0 {
				tab, ret = processInstructions(cmds.Commands, node, match, tab, ret, idx, lvl, atl, accum)
			}

			// process sub commands on child node
			for _, sub := range cmds.Subtasks {
				tab, ret = processCommands(sub, node, tab, ret, 1, lvl, atl, accum)
			}
			...
			if len(cmds.Failure) > 0 {
				tab, ret = processInstructions(cmds.Failure, node, match, tab, ret, idx, lvl, atl, accum)
			}
		}
	}
...
func ProcessExtract(text, parent string, index int, hd, tl string, transform map[string]string, srchr *FSMSearcher, histogram map[string]int, cmds *Block) string {
	...
	if cmds.Position == "select" {
		...
		atl := Atlas{Variables: variables, Transform: transform, Srchr: srchr, Histogram: histogram}
		_, ret = processCommands(cmds, pat, "", "", index, 1, atl,
			func(str string) {
				if str != "" {
					ok = true
					buffer.WriteString(str)
