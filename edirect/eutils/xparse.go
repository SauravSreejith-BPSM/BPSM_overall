...
func ParseArguments(cmdargs []string, pttrn string) *Block {
	...
	parseCommands = func(parent *Block, startLevel LevelType) {
		...
		findNextLevel := func(args []string, level LevelType) (LevelType, string) {
		...
		level, lctag := findNextLevel(arguments, startLevel)
	...
	parseConditionals := func(cmds *Block, arguments []string) []*Operation {
		...
		for idx < max {
			...
			case SELECT, AND, OR:
				...
				parseStep(op, elementColonValue)
				status = UNSET
			case EQUALS, CONTAINS, MIMICS, INCLUDES, EXCLUDES, ISWITHIN, STARTSWITH, ENDSWITH, ISNOT, ISBEFORE, ISAFTER, CONSISTSOF:
				if op != nil {
					if len(str) > 1 && str[0] == '\\' {
	...
	// add second exploration level to "-block INSDFeature -if ..." or "-block INSDFeature -pkg feat_intervals"
	// resulting in
	// "block INSDFeature -blockX INSDFeature -if ..." or  "-block INSDFeature -blockY INSDFeature -pkg feat_intervals"
	for _, txt := range cmdargs {

		if argTypeIs[txt] == EXPLORATION && txt != "-pattern" {

			// extra exploration arguments ending in X or Y are for internal use only
			if strings.HasSuffix(txt, "X") || strings.HasSuffix(txt, "Y") {
		...
		if isExplore {
			...
			// exhaustive exploration uses single asterisk to select current child
			if txt == "**" {
				txt = "*"
			}
		...
		// -if or -unless adds additional exploration level "X" (higher than -pkg command "Y")
		if (txt == "-if" || txt == "-unless") && lastLevel != "" && lastVisit != "" {
			expargs = append(expargs, lastLevel+"X")
			expargs = append(expargs, lastVisit)
		}

		// -pkg adds additional exploration level "Y" (lower than -if command "X")
		if txt == "-pkg" && lastLevel != "" && lastVisit != "" && lastVisit != "*" {
			expargs = append(expargs, lastLevel+"Y")
			expargs = append(expargs, lastVisit)
		}
...
func PrettyArguments(cmdargs []string) {
	...
	for _, str := range cmdargs {
		...
		} else if strings.Index(str, quote) >= 0 {
			fmt.Fprintf(os.Stdout, " %s", apos+str+apos)
		} else if needsQuotes(str) || str == "" || str == "-" {
...
func printXMLtree(node *XMLNode, style IndentType, mask string, printAttrs bool, proc func(string)) {
	...
	// indent a specified number of double spaces
	doIndent := func(indt int) {
		i := indt
		for i > 9 {
			proc("                    ")
