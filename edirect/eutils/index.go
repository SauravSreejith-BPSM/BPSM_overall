...
func IncrementalIndex(db, pfx, ptrn string, dotMax int, csmr e2IndexConsumer) <-chan string {
	...
	indexBase, ok := pths.GetLocalPath("Index")

	if indexBase == "" {
		DisplayError("Unable to get local index path")
		os.Exit(1)
	}
	if !ok {
		DisplayError("Local index is not mounted")
		os.Exit(1)
	}
	...
	if dotMax == 0 {
		// show a dot every 1000 .e2x files, generated from up to 100000 .xml files
		dotMax = 1000
	}
...
func IncrementalInvert(db string, dotMax int) <-chan string {
	...
	visitIndexFolders := func(indexBase, invertBase string) <-chan []string {
		...
		if dotMax == 0 {
			// show a dot every 30 .inv files that were deleted for regeneration,
			// representing from up to 3000 .e2x files, some also just regenerated
			dotMax = 30
		}
