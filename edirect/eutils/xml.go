	...
	xmlReader := func(in io.Reader, out chan<- XMLBlock) {
		...
		nextBuffer := func() ([]byte, bool, bool) {
			...
			if err != nil {
				if err != io.EOF {
					// real error.
					DisplayError("io.Reader failure: err.Error: %s", err.Error())
...
// getMixedContentString returns mixed-content string packaged in child list
func getMixedContentString(curr *XMLNode) string {
	...
	// do not call "str = html.UnescapeString(str)", it will remove encoded angle brackets (e.g., greater-than sign)
...
func ExploreElements(curr *XMLNode, mask, prnt, match, attrib string, wildcard, unescape bool, level int, proc func(string, int)) {
	...
	exploreElements = func(curr *XMLNode, skip string, lev int) {
		...
		for chld := curr.Children; chld != nil && chld.Name != ""; chld = chld.Next {
